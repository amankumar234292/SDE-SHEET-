...
ll topDown(int idx, vector<int>nums,vector<int>&dp)
{
    if(idx==0)
...
ll SpaceOptimizedBottomUp(vector<int>nums)
{
    // vector<int>dp(nums.size(),0);

    ll prev1=INT_MIN,prev2=INT_MIN,curr=INT_MIN;
    prev1=nums[0];
...
long long int houseRobber(vector<int>& valueInHouse)
{
    ...
    for(ll i=0;i<valueInHouse.size();i++)
    {
        if(i>0 and i<=valueInHouse.size()-1)
    ...
    return max(SpaceOptimizedBottomUp(temp1),SpaceOptimizedBottomUp(temp2));
}
